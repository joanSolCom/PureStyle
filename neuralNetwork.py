...
class NeuralNetwork(object):
	...
	def __init__(self, pathV, pathL, pathC):
		...
		after_lstm = model.predict(self.iD.dataset[0].reshape(1,30,268))
		print(after_lstm.shape)
		exit()

		'''
		model.add(Bidirectional(LSTM(402)))
		model.add(Dropout(0.4))
		model.add(Dense(100, activation='relu'))
		model.add(Dense(3,activation='softmax'))
		'''

		x_train = self.iD.dataset[:16000]
		y_train = to_categorical(self.iD.labels[:16000])

		x_valid = self.iD.dataset[16000:17500]
		y_valid = to_categorical(self.iD.labels[16000:17500])

		x_test = self.iD.dataset[17500:]
		...
		#nb_epoch = 50
		#model.fit(x_train, y_train, epochs=nb_epoch, validation_data=(x_valid, y_valid), batch_size=256, verbose=1)
